...
exports.validateElectiveSelection = validateElectiveSelection;
exports.getElectiveGroups = getElectiveGroups;
exports.checkMinimumElectives = checkMinimumElectives;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
/**
 * Validate elective course selection
 */
async function validateElectiveSelection(enrollmentNo, courseCode) {
    try {
        // Get course details
        ...
        const currentCount = currentRegistrations.filter((reg) => reg.course.is_elective &&
            reg.course.elective_group === course.elective_group).length;
 ...
 */
async function getElectiveGroups(branchCode, semesterNo) {
    try {
 ...
 */
async function checkMinimumElectives(enrollmentNo, branchCode, semesterNo) {
    try {
        ...
        for (const group of electiveGroups) {
            ...
            const count = registrations.filter((reg) => reg.course.is_elective &&
                reg.course.elective_group === group.group_code).length;
            if (count < group.min_selection) {
                missing.push(`${group.group_name} (need ${group.min_selection - count} more)`);
            }
        }
        return {
